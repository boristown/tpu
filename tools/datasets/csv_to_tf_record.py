...
def _kline_to_example(price_list, label_int):
  """Build an Example proto for an example.
  ...
  example = tf.train.Example(
      features=tf.train.Features(
          feature={
              'prices': _float_feature(price_list),
              'labels': _int64_feature(label_int)
          }))
  return example
...
def kline_to_example(csvline):
  ...
  Yields:
    serialized TF example if the label is in categories
  """
  prices_labels = csvline.plit(',')
  price_list = prices_labels[0:PRICE_COUNT*DIMENSION_COUNT*CHANNEL_COUNT]
  label_list = prices_labels[PRICE_COUNT*DIMENSION_COUNT*CHANNEL_COUNT:PRICE_COUNT*DIMENSION_COUNT*CHANNEL_COUNT+LABEL_COUNT*MAX_CASE]
  label_int = 0
  for i in range(MAX_CASE):
    label_int += (LABEL_COUNT**(MAX_CASE-1-i))*(int)label_list[(i+1)*LABEL_COUNT-1]
  #if label in categories:
    # ignore labels not in categories list
    #coder = ImageCoder()
    #image_buffer, height, width = _get_image_data(filename, coder)
    #del coder
  example = _kline_to_example(price_list, label_int)
  yield example.SerializeToString()
  ...
  with beam.Pipeline(RUNNER, options=opts) as p:
    ...
    for step in ['train', 'validation']:
      _ = (
          ...
          beam.FlatMap(lambda line: kline_to_example(line, LABELS))
